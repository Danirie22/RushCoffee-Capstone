...
const RegisterPage: React.FC = () => {
    ...
    const { register, sendEmailVerificationCode, currentUser, loading } = useAuth();

    //Red irect if already logged in
    React.useEffect(() => {
        if (!loading && currentUser && !showVerifyModal) {
    ...
    }, [currentUser, loading, navigate, showVerifyModal]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('🔵 Registration successful, user ID:', userProfile.uid);

            // Generate and send verification code
            console.log('🔵 Sending verification code...');
            await sendEmailVerificationCode(userProfile.uid, formData.email);

            // Show verification modal
            console.log('🔵 Opening verification modal...');
            setVerifyEmail(formData.email);
            setVerifyUserId(userProfile.uid);
    ...
    const handleVerificationClose = () => {
        setShowVerifyModal(false);
        // User can stay logged in even if they close the modal
        // They can verify later from their profile settings
    };

    const handleResendCode = async () => {
        if (verifyUserId && verifyEmail) {
            await sendEmailVerificationCode(verifyUserId, verifyEmail);
        }
    };
