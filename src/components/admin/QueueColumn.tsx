...
const QueueColumn: React.FC<QueueColumnProps> = ({ title, orders, onUpdateStatus, onCancelOrder, color }) => {
    return (
            ...
            <div className="flex-grow space-y-4 overflow-y-auto">
                <AnimatePresence>
                    {orders.map(order => (
                        ...
                        >
                            <OrderCard order={order} onUpdateStatus={onUpdateStatus} onCancelOrder={onCancelOrder} />
                        </motion.div>
                    ))}
                </AnimatePresence>
                {orders.length === 0 && (
